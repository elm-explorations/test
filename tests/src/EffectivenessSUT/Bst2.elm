module EffectivenessSUT.Bst2 exposing (delete, fromList, insert, keys, member, toList, union)
...
{-| BUG: When the key matches, instead of updating, we insert into one of the
subtrees, creating a duplicate entry.
-}
    ...
    case tree of
        ...
        Node left key val right ->
            if k < key then
                ...
                -- BUG: treat equal as less-than, inserting into left subtree
                -- instead of updating the current node
                Node (insert k v left) key val right
