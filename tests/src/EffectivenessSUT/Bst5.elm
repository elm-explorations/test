module EffectivenessSUT.Bst5 exposing (delete, fromList, insert, keys, member, toList, union)

{-| Bug #5: Key comparisons reversed in delete; only works correctly at the
root of the tree.

When k < key we go right, and when k > key we go left — the opposite of
correct. At the root this accidentally works if the key sought IS the root,
but searching deeper will traverse the wrong subtrees.
...
{-| BUG: comparisons are reversed when recursing. If k < key we go right
(should go left), and if k > key we go left (should go right).
Deletion at the matched node itself is still correct, so this only
works when the target key happens to be at the root.
-}
...
delete k tree =
    case tree of
        ...
        Node left key val right ->
            if k < key then
                -- BUG: should recurse into left, but recurses into right
                Node left key val (delete k right)

            else if k > key then
                -- BUG: should recurse into right, but recurses into left
