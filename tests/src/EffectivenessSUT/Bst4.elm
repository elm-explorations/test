...
{-| BUG: When recursing down to find the key, we return the modified subtree
directly instead of wrapping it back in a new Node with the current key.
This discards all ancestors of the deleted node.
-}
...
delete k tree =
    case tree of
        ...
        Node left key val right ->
            if k < key then
                -- BUG: returns just the modified left subtree
                delete k left

            else if k > key then
                -- BUG: returns just the modified right subtree
                delete k right
