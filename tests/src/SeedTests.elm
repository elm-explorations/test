...
fuzzTest : Test
fuzzTest =
    fuzz "It receives the expected number" int <|
        \num -> Expect.equal num expectedNum


fuzzTestAfterOneDistributed : Test
fuzzTestAfterOneDistributed =
    fuzz "This should be different than expectedNum, because there is a fuzz test before it." int <|
...
tests =
    [ describe "Seed test"
        [ fuzzTest ]
    , describe "Seed test"
        [ fuzz "It receives the expected number even though this text is different" int <|
            \num ->
    ...
    , Test.concat
        [ fuzz "top-level fuzz tests don't affect subsequent top-level fuzz tests, since they use their labels to get different seeds" int <|
            \num ->
                Expect.equal num 110
        , describe "Seed test"
            [ fuzzTest ]
        , describe "another top-level fuzz test"
            [ fuzz "it still gets different values, due to computing the seed as a hash of the label, and these labels must be unique" int <|
...
noAutoFail =
    ...
    , -- Test.skip skips the test in question
      describe "Seed test"
        [ skip <|
            fuzz "Skip test sanity check" int <|
                \_ ->
                    Expect.fail "Test.skip is broken! This should not have been run."
        , fuzzTestAfterOneDistributed
        ]
