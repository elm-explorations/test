...
values, the simplifier takes the failing value and makes it smaller/simpler for
you so you can guess more easily what property of that value caused the test
to fail.

Simplifying is a way to try and find the "smallest", "simplest" example that
fails, in order to give the tester better feedback on what went wrong.

Simplifiers are functions that, given a failing value, offer "smaller", "simpler"
...
When you write your own Simplifier, you decide what is small for the kind of data
you're testing with.
...
to tell which is responsible for the failing. If we don't attempt to simplify it,
...
A simplifier takes a value and returns a short list of smaller values.
...
will only produce simplifys which satisfy the given predicate.
-}
...
{-| Filter out the results of a simplifier. The resulting simplifier
will only throw away simplifys which satisfy the given predicate.
-}
